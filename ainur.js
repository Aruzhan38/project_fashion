...
const UX = (() => {
  function toast(msg, type = "info", timeout = 2000) {
  ...
  function beep(durationMs = 120, freq = 880, type = "sine", volume = 0.08) {
    try {
      const ctx = UX._ctx ?? new (window.AudioContext || window.webkitAudioContext)();
      UX._ctx = ctx;
  ...
  function init() {
    initScrollBar();
    document.querySelectorAll(".item, .card, .pill, .btn").forEach(el =>
      el.addEventListener("click", () => { pop(el); beep(80, 600); })
    );
  }

  return { init, toast, pop, beep, _ctx: null };
})();
